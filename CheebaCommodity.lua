-----------------------------------------------------------------------------------------------
-- Client Lua Script for CheebaCommodity
...
function CheebaCommodity:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CheebaCommodity:Init()
    Apollo.RegisterAddon(self)
end

function CheebaCommodity:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CheebaCommodity.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CheebaCommodity:OnDocumentReady()
...
function CheebaCommodity:OnDestroy()
...
function CheebaCommodity:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
	end

	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CheebaCommodityForm", nil, self)
...
function CheebaCommodity:InitializeCategories()
...
function CheebaCommodity:OnResizeCategories() -- Can come from XML
...
function CheebaCommodity:OnHeaderBtnToggle()
	-- Filters
...
function CheebaCommodity:InitializeSell()
...
function CheebaCommodity:InitializeBuy()
...
function CheebaCommodity:OnSearchEditBoxChanged(wndHandler, wndControl) -- SearchEditBox
	self.wndMain:FindChild("SearchClearBtn"):Show(string.len(wndHandler:GetText() or "") > 0)
end

function CheebaCommodity:OnSearchClearBtn(wndHandler, wndControl)
	self.wndMain:FindChild("SearchEditBox"):SetText("")
	self.wndMain:FindChild("SearchClearBtn"):Show(false)
	self:OnSearchCommitBtn()
end

function CheebaCommodity:OnSearchCommitBtn(wndHandler, wndControl) -- ALso SearchEditBox's WindowKeyReturn
...
function CheebaCommodity:OnRefreshBtn(wndHandler, wndControl) -- Also from lua and multiple XML buttons
	self.wndMain:FindChild("RefreshAnimation"):SetSprite("CRB_WindowAnimationSprites:sprWinAnim_BirthSmallTemp")
	self:OnHeaderBtnToggle()
end
...
function CheebaCommodity:BuildListItem(tCurrItem, strWindowName, strBtnText)
...
function CheebaCommodity:OnListInputNumberChanged(wndHandler, wndControl, strText)
...
function CheebaCommodity:OnListInputNumberUpBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CheebaCommodity:OnListInputNumberDownBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CheebaCommodity:OnListInputNumberHelper(wndParent, nNewValue)
...
function CheebaCommodity:OnListInputPriceAmountChanged(wndHandler, wndControl) -- ListInputPrice, data is parent
...
function CheebaCommodity:OnListInputPriceMouseDown(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", false)
	self:HelperValidateListInputForSubmit(wndHandler:GetData())
end

function CheebaCommodity:OnListInputPriceLoseFocus(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", true)
	self:HelperValidateListInputForSubmit(wndHandler:GetData())
end

function CheebaCommodity:HelperValidateListInputForSubmit(wndParent)
	local nAvailable = 0
	local nQuantity = 0
	local nPrice = 0
...
function CheebaCommodity:OnFilterOptionsLevelUpBtn(wndHandler, wndControl)
...
function CheebaCommodity:OnFilterOptionsLevelDownBtn(wndHandler, wndControl)
	local wndEditBox = wndHandler:GetParent():FindChild("FilterOptionsLevelEditBox")
	local nOldValue = tonumber(wndEditBox:GetText())
	local nNewValue = nOldValue and nOldValue - 1
	wndEditBox:SetText(nNewValue)
...
function CheebaCommodity:OnFilterEditBoxChanged(wndHandler, wndControl)
	local wndEditBox = wndHandler:GetParent():FindChild("FilterOptionsLevelEditBox")
	self:HelperCheckValidLevelValues(wndEditBox)
...
function CheebaCommodity:OnFilterOptionsRarityItemToggle(wndHandler, wndControl) -- FilterOptionsRarityItemBtn
...
function CheebaCommodity:OnResetFilterBtn(wndHandler, wndControl)
	local wndFilter = self.wndMain:FindChild("FilterContainer")
...
function CheebaCommodity:OnFilterOptionsWindowClosed(wndHandler, wndControl)
...
function CheebaCommodity:HelperCheckValidLevelValues(wndChanged)
...
function CheebaCommodity:OnCategoryTopBtnToggle(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CheebaCommodity:OnCategoryMidBtnCheck(wndHandler, wndControl)
...
function CheebaCommodity:OnGenerateSimpleConfirmTooltip(wndHandler, wndControl, eType, nX, nY) -- wndHandler is ListSubmitBtn, data is { tCurrItem and window "SimpleListItem" }
	...
	local bBuyNow = self.wndMain:FindChild("HeaderBuyNowBtn"):IsChecked()
	local wndTooltip = wndHandler:LoadTooltipForm("CheebaCommodity.xml", "SimpleConfirmTooltip", self)
...
function CheebaCommodity:OnGenerateAdvancedConfirmTooltip(wndHandler, wndControl, eType, nX, nY)
	...
	local wndTooltip = wndHandler:LoadTooltipForm("CheebaCommodity.xml", "AdvancedConfirmTooltip", self)
...
function CheebaCommodity:OnGenerateTooltipFullStats(wndHandler, wndControl, eType, nX, nY) -- GOTCHA: wndHandler is ListSubtitle
	...
	local nLastCount = 0
	local wndFullStats = wndHandler:LoadTooltipForm("CheebaCommodity.xml", "FullStatsFrame", self)
	for nRowIdx = 1, 3 do
		local strBuy = ""
...
function CheebaCommodity:OnGenerateTooltipListIcon(wndHandler, wndControl, eType, nX, nY)
...
function CheebaCommodity:OnCommodityInfoResults(nItemId, tStats, tOrders)
	...
	elseif self.wndMain:FindChild("HeaderSellNowBtn"):IsChecked() then
		...
		if self.wndMain:FindChild("HeaderBuyOrderBtn"):IsChecked() then
			nValueForInput = (nSmall and nSmall+1 or nSmall)
			for nIdx, tCurrOrder in pairs(self.tOrders) do
				local tItem = tCurrOrder:GetItem()
				local tItemId = tItem:GetItemId()
				if(tItemId == nItemId and tCurrOrder:IsBuy() and nSmall == tCurrOrder:GetPricePerUnit():GetAmount()) then
						nValueForInput = nSmall
					break
				end
			end
		...
		elseif self.wndMain:FindChild("HeaderSellOrderBtn"):IsChecked() then
			nValueForInput =  (nBig and nBig-1 or nBig)
			for nIdx, tCurrOrder in pairs(self.tOrders) do
				local tItem = tCurrOrder:GetItem()
				local tItemId = tItem:GetItemId()
				if(tItemId == nItemId and not tCurrOrder:IsBuy() and nBig == tCurrOrder:GetPricePerUnit():GetAmount()) then
						nValueForInput = nBig
					break
				end
			end
...
function CheebaCommodity:OnPostCommodityOrderResult(eAuctionPostResult, orderSource, nActualCost)
...
function CheebaCommodity:OnPostCustomMessage(strMessage, bResultOK, nDuration)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CheebaCommodity:OnCommodityAuctionRemoved(eAuctionEventType, orderRemoved)
	-- TODO
...
function CheebaCommodity:OnPostResultTimer()
	if self.wndMain and self.wndMain:IsValid() then
...
function CheebaCommodity:OnCommodityDataReceived(tOrders) -- From MarketplaceLib.RequestOwnedCommodityOrders()
	self:UpdateOrderLimit(#tOrders)
	self.tOrders = tOrders
end

function CheebaCommodity:UpdateOrderLimit(nCount)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
	if nCount < 0 then
...
function CheebaCommodity:OnOpenMarketListingsBtn(wndHandler, wndControl)
...
function CheebaCommodity:OnPostResultNotificationClick(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:Show(false)
	end
end

function CheebaCommodity:LoadByName(strForm, wndParent, strCustomName)
...
local CheebaCommodityInst = CheebaCommodity:new()
CheebaCommodityInst:Init()
